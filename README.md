...
```
# Decode audio segments via emulation of firmware routines
uv run emu_decoder.py firmware.bin
# Decode audio segments using standalone decoder (broken)
uv run standalone_decoder.py firmware.bin
```

## Overview

This firmware is for a Nuvoton ISD9160 Soundcorder chip (ARM Cortex-M0). The VPE (Voice Playback Engine) section contains 9 audio segments with a **custom format** (not standard Nuvoton VPE library).
